...
#define UPDATE_DELAY 100000 // 0.1 seconds in microseconds
#define NUM_LINES 3
...
void draw_legend(WINDOW *win, DataLine lines[], int num_lines, int paused) {
  int start_y = 1;
  ...
  mvwprintw(win, start_y + num_lines + 3, start_x + 2, "Space: %s",
            paused ? "Resume" : "Pause");
  mvwprintw(win, start_y + num_lines + 4, start_x + 2, "Q: Quit");
...
int main() {
  ...
  while (1) {
    ...
    draw_legend(win, lines, NUM_LINES, paused);
