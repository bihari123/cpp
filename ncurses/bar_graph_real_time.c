...
#define UPDATE_INTERVAL 100000 // 0.1 seconds in microseconds
...
int main() {
  ...
  cbreak();
  keypad(stdscr, TRUE);
  timeout(0); // Set getch() to non-blocking
  ...
  while (1) {
    ...
    // Update bar values periodically
    if (highlight == 0) { // Only update when on the bar graph tab
      update_bar_values();
    }
...
void draw_menu(WINDOW *menu_win, int highlight) {
  int x, y, i;
  char *tabs[NUM_TABS] = {"Real-time Graph", "Tab 2", "Tab 3"};
...
void draw_bar_graph(WINDOW *menu_win) {
  ...
  mvwprintw(menu_win, start_y - 1, start_x, "Real-time Bar Graph:");
...
void update_bar_values() {
  for (int i = 0; i < NUM_BARS; i++) {
    // Randomly increase or decrease the bar value
    ...
    if (bar_values[i] < 0)
      bar_values[i] = 0;
    if (bar_values[i] > MAX_BAR_HEIGHT)
      bar_values[i] = MAX_BAR_HEIGHT;
  }
}
